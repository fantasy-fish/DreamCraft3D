...
for image_id in "${image_ids[@]}"; do
    # if [[ " ${skip_list[*]} " =~ " ${image_id} " ]]; then
    #     continue 
    # fi

    ((counter++))
    echo "Processing ${counter}-th file: ${image_id}"
    image_path_old="${DIRECTORY}${image_id}.png"
    image_path="${DIRECTORY}${image_id}_rgba.png"
    echo "$image_path"
    # --------- Stage 0 preprocess --------- # 
    ...
    caption_path="${DIRECTORY}${image_id}_caption.txt"
